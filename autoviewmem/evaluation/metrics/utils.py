...
def _normalize_answer_for_em(s: str) -> str:
    """
    更稳健的 EM 归一化（可按需简化/增强）：
    - 小写
    - 去首尾空格
    - 合并多空格
    - 去掉常见标点/括号（避免 “Paris.” vs “Paris” 不匹配）
    """
    s = str(s).strip().lower()
    s = re.sub(r"\s+", " ", s)
    # 去掉常见标点（保守一些，避免误伤数字/小数点的话可以去掉 '.' 的处理）
    s = re.sub(r"[，,。.!！？?；;：“”\"'‘’（）()\[\]{}<>]", "", s)
    s = s.strip()
    return s


def calculate_em_reward(prediction: str, reference: str, *, normalize: bool = True) -> float:
    """
    EM reward: 完全匹配则 1.0，否则 0.0
    normalize=True 时做轻量归一化，适合提升鲁棒性
    """
    ...
    if normalize:
        pred = _normalize_answer_for_em(pred)
        ref = _normalize_answer_for_em(ref)
    else:
        pred = pred.strip()
        ref = ref.strip()
