...
from autoviewmem import config
from autoviewmem.config import LLM_MAX_TOKENS, LLM_TEMPERATURE
...
class HybridRetriever:
    ...
    def _retrieve_with_reflection(self, query: str, initial_results: List[MemoryEntry]) -> List[MemoryEntry]:
        ...
        for round_num in range(self.max_reflection_rounds):
            ...
            adequacy = self._check_answer_adequacy(query, current_results)
            assessment = (adequacy.get("assessment") or "Insufficient").strip()
            ...
            additional_queries = self._generate_additional_queries(query, current_results, missing_info)
            ...
            current_results = self._merge_and_deduplicate_entries(current_results + additional_results)
...
"""
    ...
    def _check_answer_adequacy(self, query: str, contexts: List[MemoryEntry]) -> Dict[str, Any]:
        """
        Check if current contexts are sufficient to answer the query
        Returns: {"assessment": "Sufficient"|"Insufficient", "reasoning": str, "missing_info": [..]}
        """
        if not contexts:
            return {"assessment": "Insufficient", "reasoning": "No contexts available", "missing_info": ["relevant memory evidence"]}
...
"""
        ...
        try:
            ...
            result = self.llm_client.extract_json(response)
            assessment = (result.get("assessment") or "Insufficient").strip()
            ...
            return {"assessment": "Insufficient", "reasoning": str(e), "missing_info": ["relevant memory evidence"]}
    
    def _generate_additional_queries(self, original_query: str, current_contexts: List[MemoryEntry], missing_info: List[str]) -> List[str]:
        """
...
"""
        ...
        try:
            ...
            result = self.llm_client.extract_json(response)
            queries = result.get("additional_queries", []) or []
            out = []
            seen = set()
            for q in queries:
                s = (q or "").strip()
...
"""
    ...
    def _generate_targeted_queries(self, original_query: str, information_plan: Dict[str, Any]) -> List[str]:
        """
        Generate 3-5 targeted queries based on information requirements analysis
        """
        prompt = f"""
...
"""
        ...
        try:
            ...
            if len(queries) < 3:
                key_entities = information_plan.get("key_entities", []) or []
                relationships = information_plan.get("relationships", []) or []
                for ent in key_entities[:3]:
                    queries.append(f"{ent} {original_query}")
                for rel in relationships[:2]:
                    queries.append(f"{original_query} {rel}")
                queries = _dedupe_keep_order(queries)
